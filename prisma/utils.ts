...
/**
 * Determines whether the interview has ended.
 *
 * This function calculates the end time of the interview by adding the specified duration
 * (in minutes) to the start time provided. It then compares the end time with the current time.
 *
 * @param startTime - The start time of the interview as a date string acceptable by the Date constructor.
 * @param duration - The duration of the interview in minutes.
 * @returns True if the current time is later than the computed end time, indicating the interview has ended; otherwise, false.
 */
export function isInterviewEnded(startTime: Date, duration: number) {
