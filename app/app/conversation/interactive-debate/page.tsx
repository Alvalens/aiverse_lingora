...
export default function CreateConversationPage() {
  ...
  const generateThemes = async () => {
    ...
    try {
      const response = await fetch("/api/conversations/interactive-debate/theme", {
        method: "POST",
        headers: {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-primary">
      <div className="w-full max-w-2xl p-6 bg-[#052038] rounded-lg shadow-lg text-white">
        ...
        {/* History Section */}
        <div>
          <h2 className="text-lg font-semibold mb-2 text-color-text">Your Debate History</h2>
          {isHistoryLoading ? (
            ...
            <ul className="space-y-4 max-h-64 overflow-y-auto">
              {history.map((item) => (
                <li
                  ...
                  onClick={() => router.push(`/app/conversation/interactive-debate/${item.id}/result`)}
                >
